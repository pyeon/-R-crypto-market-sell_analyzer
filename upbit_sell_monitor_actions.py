...
# -*- coding: utf-8 -*-
"""
업비트 실시간 매도 신호 모니터링 시스템 v2.0 - GitHub Actions Edition
개선 사항:
- GitHub Actions 실행 최적화 (API 호출 → 파일 저장 → Git push → 텔레그램 요약)
- 10분봉 + 60분봉 혼합 분석으로 급격한 변동 감지
- JSON/Excel/Markdown 다중 포맷 저장
"""
...
def save_to_json(results):
    """분석 결과를 JSON으로 저장"""
    try:
        json_file = MARKET_DATA_DIR / "upbit_sell_signals_latest.json"
        
        data = {
            'timestamp': format_kst_time(),
...
def generate_markdown_report(results):
    ...
    try:
        now = get_kst_now()
        md_file = REPORTS_DIR / f"upbit_sell_signals_{now.strftime('%Y%m%d_%H%M')}.md"
        
        content = f"""# 업비트 매도 신호 분석 리포트

## 📊 분석 개요
- **분석 시각**: {format_kst_time()}
- **분석 대상**: {results['total_analyzed']}개 코인
- **매도 신호**: {results['signals_found']}개 발견
...
"""
        ...
        if results['signals_found'] > 0:
            # 즉시매도
            immediate = [s for s in results['signals'] if s['stage'] == '즉시매도']
            if immediate:
                content += "## 🔴 즉시매도 ({len(immediate)}개)\n\n"
                for signal in immediate:
                    content += f"### {signal['coin']}\n"
                    content += f"- **현재가**: {signal['current_price']:,.0f}원\n"
                    content += f"- **신호강도**: {signal['score']}/10\n"
                    content += f"- **주요지표**:\n"
            ...
            # 매도준비
            prepare = [s for s in results['signals'] if s['stage'] == '매도준비']
            if prepare:
                content += f"## 🟠 매도준비 ({len(prepare)}개)\n\n"
                for signal in prepare:
                    content += f"### {signal['coin']}\n"
                    content += f"- **현재가**: {signal['current_price']:,.0f}원\n"
                    content += f"- **신호강도**: {signal['score']}/10\n"
                    content += f"- **주요지표**: {', '.join(signal['signals'][:3])}\n\n"
            
            # 매도검토
            review = [s for s in results['signals'] if s['stage'] == '매도검토']
            if review:
                content += f"## 🟡 매도검토 ({len(review)}개)\n\n"
                for signal in review:
                    content += f"- **{signal['coin']}**: {signal['current_price']:,.0f}원 (신호: {signal['score']}/10)\n"
                content += "\n"
        else:
            content += "## ✅ 매도 신호 없음\n\n현재 조건에 맞는 매도 신호가 발견되지 않았습니다.\n\n"
        
        content += "---\n\n"
        content += "*이 리포트는 자동으로 생성되었습니다. 투자 판단은 신중히 하시기 바랍니다.*\n"
...
def save_to_excel(results):
    ...
    try:
        ...
        for signal in results['signals']:
            pattern = signal.get('pattern_data', {})
            volume = signal.get('volume_data', {})
            orderbook = signal.get('orderbook_data', {})
            ...
            row_data = [
                ...
                f"-{pattern.get('quick_drop', 0):.1f}% ({pattern.get('minutes_since_high', 0)}분)" if pattern else '',
                f"-{pattern.get('drop_from_high_12h', 0):.1f}%" if pattern else '',
                f"{pattern.get('surge_6h', 0):+.1f}%" if pattern else '',
                "감소" if volume.get('volume_declining') else "정상",
                "있음" if volume.get('divergence_signal') else "없음",
                f"{orderbook.get('ask_bid_ratio', 0):.2f}" if orderbook else '',
                f"{indicators.get('rsi', 0):.1f}" if indicators else '',
                indicators.get('macd_signal', '') if indicators else '',
                indicators.get('bb_signal', '') if indicators else '',
                indicators.get('ma_signal', '') if indicators else '',
                f"{indicators.get('stoch', 0):.1f}" if indicators else ''
            ]
...
def send_summary_telegram(results):
    """분석 완료 후 요약만 텔레그램 전송"""
    try:
        now = format_kst_time()
        
        message = f"🔍 업비트 매도 신호 분석 완료\n"
        message += "━━━━━━━━━━━━━━━━━━━━━\n"
        message += f"⏰ 분석시각: {now}\n"
        message += f"📊 분석대상: {results['total_analyzed']}개 코인\n"
        message += f"🎯 매도신호: {results['signals_found']}개 발견\n\n"
        
        if results['signals_found'] > 0:
            # 단계별 집계
            immediate = len([s for s in results['signals'] if s['stage'] == '즉시매도'])
            prepare = len([s for s in results['signals'] if s['stage'] == '매도준비'])
            review = len([s for s in results['signals'] if s['stage'] == '매도검토'])
            
            message += "【 단계별 분류 】\n"
            if immediate > 0:
                message += f"🔴 즉시매도: {immediate}개\n"
            if prepare > 0:
                message += f"🟠 매도준비: {prepare}개\n"
            if review > 0:
                message += f"🟡 매도검토: {review}개\n"
            
            message += "\n【 주요 매도 신호 】\n"
            
            # 즉시매도 상세
            for signal in results['signals']:
                if signal['stage'] == '즉시매도':
                    message += f"\n🔴 {signal['coin']}\n"
                    message += f"💰 {signal['current_price']:,.0f}원 ({signal['score']}/10)\n"
                    message += f"📌 {', '.join(signal['signals'][:3])}\n"
            
            # 매도준비 간략
            prepare_coins = [s['coin'] for s in results['signals'] if s['stage'] == '매도준비']
            if prepare_coins:
                message += f"\n🟠 매도준비: {', '.join(prepare_coins[:5])}"
                if len(prepare_coins) > 5:
                    message += f" 외 {len(prepare_coins)-5}개"
                message += "\n"
        else:
            message += "✅ 현재 조건에 맞는 매도 신호 없음\n"
        
        message += "\n━━━━━━━━━━━━━━━━━━━━━\n"
        message += "📁 상세 분석 결과:\n"
        message += "- JSON: market_data/\n"
        message += "- Excel: upbit_sell_signals_v2.xlsx\n"
        message += "- Report: analysis_reports/\n"
        
        send_telegram(message)
        print("✅ 텔레그램 요약 전송 완료")
        
    except Exception as e:
        print(f"텔레그램 요약 전송 오류: {e}")
...
def scan_sell_signals():
    """매도 신호 스캔 (GitHub Actions 최적화)"""
    print(f"\n{'='*50}")
    print(f"🔍 매도 신호 스캔 시작: {format_kst_time()}")
    print(f"{'='*50}\n")
        ...
        try:
            ...
            if score >= SELL_STAGE_REVIEW:
                ...
                if stage_info:
                    ...
                    print(f"✅ 매도신호 감지: {coin} ({stage_info['stage']}, {score}/10)")
    ...
    print(f"\n{'='*50}")
    print(f"✅ 스캔 완료: 총 {results['signals_found']}개 매도신호 발견")
    print(f"{'='*50}\n")
...
def main():
    ...
    try:
        ...
        print(f"\n❌ 오류 발생: {e}")
        send_telegram(f"❌ 업비트 매도 신호 분석 오류\n{str(e)}\n시각: {format_kst_time()}")
